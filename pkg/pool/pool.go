...
func (pool *Pool) submitNonce(nonceSubmission *NonceSubmission) {
	...
	for try := 0; try < nonceSubmissionRetries; try++ {
		Logger.Info("submitting: ", zap.Uint64("Nonce", nonceSubmission.Nonce),
			zap.Uint64("Miner", nonceSubmission.MinerID),
			zap.Uint64("Deadline", nonceSubmission.Deadline))
...
func (pool *Pool) processSubmitNonceRequest(w http.ResponseWriter, req *http.Request) {
	...
	requestLogger.Info("Input:", zap.Uint64("nonce", nonce), zap.Uint64("baseTarget", ri.BaseTarget))
	...
	requestLogger.Info("Output:", zap.Uint64("deadline", deadline))
