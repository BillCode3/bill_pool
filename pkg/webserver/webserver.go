...
func (webServer *WebServer) blockInfoHandler(w http.ResponseWriter, r *http.Request) {
	blockInfo := webServer.getBlockInfo()
	res, err := json.Marshal(&blockInfo)
	if err != nil {
		fmt.Fprintf(w, "{'errCode':'-1'}")
		return
	}
	fmt.Fprintf(w, string(res))
}

func (webServer *WebServer) poolInfoHandler(w http.ResponseWriter, r *http.Request) {
	Logger.Info("Pool info")
	poolInfo := webServer.getPoolStatsInfo()
	res, err := json.Marshal(&poolInfo)
	if err != nil {
		fmt.Fprintf(w, "{'errCode':'-1'}")
		return
	}
...
func (webServer *WebServer) poolConfigInfoHandler(w http.ResponseWriter, r *http.Request) {
	poolConfigInfo := &api.PoolConfigInfo{
		...
		Version:         Cfg.Version,
		PoolPublicID:    Cfg.PoolPublicID}
	res, err := json.Marshal(poolConfigInfo)
	if err != nil {
		fmt.Fprintf(w, "{'errCode':'-1'}")
		return
	}
...
func (webServer *WebServer) minerInfoHandler(w http.ResponseWriter, r *http.Request) {
	res, err := json.Marshal(webServer.minerInfos)
	if err != nil {
		fmt.Fprintf(w, "{'errCode':'-1'}")
		return
	}
...
func (webServer *WebServer) listen() {
	...
	http.HandleFunc("/blockInfo", webServer.blockInfoHandler)
	http.HandleFunc("/poolInfo", webServer.poolInfoHandler)
	http.HandleFunc("/poolConfigInfo", webServer.poolConfigInfoHandler)
	http.HandleFunc("/minerInfo", webServer.minerInfoHandler)
